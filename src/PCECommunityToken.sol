    ...
    TokenSetting
{
    ...
    function rawBalanceToDisplayBalance(
        uint256 rawBalance
    ...
    function _beforeTokenTransfer(
        ...
        if (midnightTotalSupplyModifiedTime == 0) {
            ...
            intervalDaysOf(midnightTotalSupplyModifiedTime, block.timestamp, 1)
        ) {
    ...
    function _beforeTokenTransferAtAddress(address account) internal {
        if (_accountInfos[account].firstTransactionTime == 0) {
            ...
            _accountInfos[account].lastModifiedMidnightBalanceTime = block
                .timestamp;
            _accountInfos[account].midnightBalance = super.balanceOf(account);
        } else if (
            intervalDaysOf(
                _accountInfos[account].lastModifiedMidnightBalanceTime,
                block.timestamp,
                1
            )
        ) {
    ...
    function _isAllowExchange(
        bool isIncome,
        address tokenAddress
    ) private view returns (bool) {
        ExchangeAllowMethod allowMethod = isIncome
            ? incomeExchangeAllowMethod
            : outgoExchangeAllowMethod;
        address[] memory targetTokens = isIncome
            ? incomeTargetTokens
            : outgoTargetTokens;
    ...
    function isAllowOutgoExchange(
        address tokenAddress
    ) public view returns (bool) {
        return _isAllowExchange(false, tokenAddress);
    }

    function isAllowIncomeExchange(
        address tokenAddress
    ) public view returns (bool) {
    ...
    */
    ...
    function swapTokens(address toTokenAddress, uint256 amountToSwap) public {
        ...
        Utils.LocalToken memory fromToken = pceToken.getLocalToken(
            address(this)
        );
        require(fromToken.isExists, "From token not found");

        Utils.LocalToken memory toToken = pceToken.getLocalToken(
            toTokenAddress
        );
        ...
        require(
            isAllowOutgoExchange(toTokenAddress),
            "Outgo exchange not allowed"
        );
        require(
            to.isAllowIncomeExchange(address(this)),
    ...
    */
    ...
    function transferWithAuthorization(
        ...
        emit MetaTransactionFeeCollected(
            from,
            _msgSender(),
            displayFee,
            rawFee
