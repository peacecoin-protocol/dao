    ...
    IPCEToken
{
    ...
    struct TokenInfo {
        ...
        address[] incomeTargetTokens;
        address[] outgoTargetTokens;
    }
    ...
    ) public view returns (Utils.LocalToken memory) {
    ...
    function createToken(TokenInfo calldata tokenInfo) public {
        ...
        require(
            ...
            "Insufficient PCEToken bal."
        );
        require(
            tokenInfo.dilutionFactor >= 10 ** 17 &&
                tokenInfo.dilutionFactor <= 10 ** 21,
            "Dilution factor 0.1-1000"
        );
        require(
    ...
    function swapToLocalToken(address toToken, uint256 amountToSwap) public {
        ...
        require(localTokens[toToken].isExists, "Target token not found");
        require(
            balanceOf(_msgSender()) >= amountToSwap,
            "Not enough PCEToken balance"
        );
        ...
        localTokens[toToken].depositedPCEToken =
            localTokens[toToken].depositedPCEToken +
            amountToSwap;

        emit TokensSwappedToLocalToken(
            _msgSender(),
            toToken,
            amountToSwap,
            targetTokenAmount
    ...
    function swapFromLocalToken(
        ...
        require(
            target.balanceOf(_msgSender()) >= amountToSwap,
        ...
        emit TokensSwappedFromLocalToken(
            _msgSender(),
            fromToken,
            amountToSwap,
            pcetokenAmount
    ...
    */
    ...
    function setNativeTokenToPceTokenRate(
        uint160 _nativeTokenToPceTokenRate
    ) public onlyOwner {
        nativeTokenToPceTokenRate = _nativeTokenToPceTokenRate;
    }

    function setMetaTransactionGas(
        uint256 _metaTransactionGas
    ) public onlyOwner {
        metaTransactionGas = _metaTransactionGas;
    }

    function setMetaTransactionPriorityFee(
